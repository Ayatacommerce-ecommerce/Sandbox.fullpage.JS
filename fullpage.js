setTimeout(() => {
        ...
        function initialise(containerSelector, options) {
                    ...
                    if (
                        ...
                        anchors.forEach(function (item) {
                            options.anchors.push(
                                item.getAttribute("data-anchor").toString()
                            );
                        });
                    }
                }
            ...
            function createSlideArrows(section) {
                var arrows = [
                    createElementFromHTML(
                        '<div class="' + SLIDES_ARROW_PREV + '"></div>'
                    ),
                    createElementFromHTML(
                        '<div class="' + SLIDES_ARROW_NEXT + '"></div>'
                    ),
                ];
          ...
          */
            ...
            function moveSlide(direction, section) {
                var activeSection =
                    section == null ? $(SECTION_ACTIVE_SEL)[0] : section;
            ...
            function getDestinationPosition(element) {
                ...
                else if (isResizing && next(element) == null) {
            ...
            function fireCallback(eventName, v) {
                ...
                if (!options.v2compatible) {
                    ...
                    if (
                        options[eventName].apply(eventData[0], eventData.slice(1)) ===
                        false
                    ) {
                        return false;
                    }
            ...
            function createInfiniteSections(v) {
                ...
                if (!v.isMovementUp) {
                    ...
                    before(
                        $(SECTION_ACTIVE_SEL)[0],
                        nextAll(v.activeSection, SECTION_SEL)
                    );
                }
            ...
            function continuousVerticalFixSectionOrder(v) {
                ...
                if (v.isMovementUp) {
                    ...
                    after(
                        $(SECTION_SEL)[$(SECTION_SEL).length - 1],
            ...
            function lazyLoadOthers() {
                ...
                $(SECTION_SEL + ":not(" + ACTIVE_SEL + ")").forEach(function (
                    section
                ) {
            ...
            function playMedia(destiny) {
                ...
                //youtube videos
                $('iframe[src*="youtube.com/embed/"]', panel).forEach(function (
                    element
                ) {
            ...
            function responsive() {
                ...
                var isBreakingPointWidth =
                    widthLimit && window.innerWidth < widthLimit;
            ...
            function silentLandscapeScroll(activeSlide, noCallbacks) {
                ...
                landscapeScroll(
                    closest(activeSlide, SLIDES_WRAPPER_SEL),
                    activeSlide
                );
            ...
            function Item(el, selector) {
                ...
                this.isLast =
                    this.index ===
                    el.parentElement.querySelectorAll(selector).length - 1;
    ...
    */
    ...
    // Preload the images for first two sections
    for (let s = 1; s <= 2; s++) {
        for (let i = 1; i <= 30; i++) {
            const img = new Image();
    ...
    /*
     * Update the canvas onload image
     */
    img.onload = function () {
    ...
    /*
     * Add Entry animation
     * this fuction animate the image sequence
     */
    ...
    /*
     * wrap the parent of shogunSection with another dom element
     * so that we can use the newly created dom elements as fullpage section
     */
    var shogunSections = $(".shogunSection");
    var fullPageContainer = $(".fullPageContainer .shg-box-content")[0];

    for (var i = 0; i < shogunSections.length; i++) {
        var section = shogunSections[i];

        const upperSection = document.createElement("div");

        upperSection.className = "section fp-auto-height";
        upperSection.appendChild(section.parentElement);
        fullPageContainer.appendChild(upperSection);
    }
    ...
    /*
     * Remove all fullpage sections while user click any other shogun-tab
     */

    const shoGunTabs = $(".shogun-tab");
    for (let i = 0; i < shoGunTabs.length; i++) {
        const tab = shoGunTabs[i];
